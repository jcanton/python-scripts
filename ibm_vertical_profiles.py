...
grid_file_path = (
    main_dir + "testdata/grids/gauss3d_torus/Torus_Triangles_1000m_x_1000m_res10m.nc"
)
...
for k, output_file in enumerate(output_files):
    ...
    axs = fig.subplots(
        nrows=len(profiles), ncols=2, sharex="col", sharey=True, squeeze=False
    )
    ...
    for i, profile in enumerate(profiles):
        ...
        axs[i, 0].set_title(
            f"(x,y) = ({tri.cell_x[cell_id]:.1f}, {tri.cell_y[cell_id]:.1f})m"
        )
    ...
    plt.draw()
    plt.savefig(
        imgs_dir + f"/v_profiles_{output_file.split(sep='.')[0]}_500m_exp.png", dpi=600
    )
    axs[0, 0].set_ylim([0, 300])
    plt.draw()
    plt.savefig(
        imgs_dir + f"/v_profiles_{output_file.split(sep='.')[0]}_300m_exp.png", dpi=600
    )
    axs[0, 0].set_ylim([0, 150])
    plt.draw()
    plt.savefig(
        imgs_dir + f"/v_profiles_{output_file.split(sep='.')[0]}_150m_exp.png", dpi=600
    )
