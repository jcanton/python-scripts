...
from icon4py.model.common.dimension import C2V, C2VDim, Koff
...
def _interpolate_from_vertices_to_cell(
    ...
    cell_field = 1.0 / 3.0 * neighbor_sum(vertex_field(C2V), axis=C2VDim)
    return cell_field
...
class Plot:
    def __init__(
        ...
        self.grid_savepoint = data_provider.from_savepoint_grid(
        ...
        self.grid = self.grid_savepoint.construct_icon_grid(backend=self._backend)
        self.interpolation_savepoint = data_provider.from_interpolation_savepoint()

        self._edge_2_cell_vector_rbf_interpolation = (
            edge_2_cell_vector_rbf_interpolation.with_backend(self._backend)
        )
        self._mo_math_divrot_rot_vertex_ri_dsl = (
            mo_math_divrot_rot_vertex_ri_dsl.with_backend(self._backend)
        )
        self._interpolate_to_full_levels = (
            interpolate_from_half_to_full_levels.with_backend(self._backend)
        )
        self._interpolate_from_vertices_to_cell = (
            interpolate_from_vertices_to_cell.with_backend(self._backend)
        )
        ...
        self.Y_BOUNDARY_RAD = (
            15 / 2 * np.pi / 180
        )  # Hardcoded in the grid generation script (could get from vertex lat)
    ...
    def rad2cart(
        self, vert_lon: np.ndarray, vert_lat: np.ndarray
    ) -> tuple[np.ndarray, np.ndarray]:
        vert_x = (
            vert_lon * self.DOMAIN_LENGTH / (2.0 * self.X_BOUNDARY_RAD)
            + self.DOMAIN_LENGTH / 2.0
        )
        vert_y = (
            vert_lat * self.DOMAIN_HEIGHT / (2.0 * self.Y_BOUNDARY_RAD)
    ...
    def _remove_boundary_triangles(
        ...
        def check_three_numbers(numbers):
            ...
            return (positive_count == 2 and negative_count == 1) or (
        ...
        def check_wrapping(vert_x, vert_y):
            ...
            return (
                check_three_numbers(vert_x)
                and (np.abs(vert_x) == self.X_BOUNDARY_RAD).any()
            ) or (
                check_three_numbers(vert_y)
                and (np.abs(vert_y) == self.Y_BOUNDARY_RAD).any()
            )
        ...
        if mask_edges:
            ...
            for i, edge in enumerate(tri.all_edges):
                if any(
                    np.array_equal(edge, filtered_edge) for filtered_edge in tri.edges
                ):
                    edges_mask[i] = False
    ...
    def _create_torus_triangulation(
        ...
        if grid_file is None:
            ...
            vert_x = np.where(
                np.abs(vert_x - self.X_BOUNDARY_RAD) < 1e-14,
                self.X_BOUNDARY_RAD,
                vert_x,
            )
            vert_x = np.where(
            ...
            vert_x = np.where(
                vert_x == self.X_BOUNDARY_RAD, -self.X_BOUNDARY_RAD, vert_x
            )
            # Adjust y values to coincide with the periodic boundary
            vert_y = np.where(
                np.abs(vert_y - self.Y_BOUNDARY_RAD) < 1e-14,
                self.Y_BOUNDARY_RAD,
                vert_y,
            )
            vert_y = np.where(
    ...
    def _vec_interpolate_to_cell_center(self, vn):
        if type(vn) is np.ndarray:
            ...
            vn_gtx = vn
        u_gtx = data_alloc.zero_field(self.grid, dims.CellDim, dims.KDim)
        v_gtx = data_alloc.zero_field(self.grid, dims.CellDim, dims.KDim)
        self._edge_2_cell_vector_rbf_interpolation(
            p_e_in=vn_gtx,
    ...
    def _compute_vt(self, vn):
        if type(vn) is np.ndarray:
            ...
            vn_gtx = vn
        vt_gtx = data_alloc.zero_field(self.grid, dims.EdgeDim, dims.KDim)
    ...
    def _compute_vorticity(self, vn):
        if type(vn) is np.ndarray:
            ...
            vn_gtx = vn
        vertvort_gtx = data_alloc.zero_field(self.grid, dims.VertexDim, dims.KDim)
        cellvort_gtx = data_alloc.zero_field(self.grid, dims.CellDim, dims.KDim)
        self._mo_math_divrot_rot_vertex_ri_dsl(
            vec_e=vn_gtx,
    ...
    def _scal_interpolate_to_full_levels(self, w_half):
        if type(w_half) is np.ndarray:
            ...
            w_half_gtx = w_half
        w_full_gtx = data_alloc.zero_field(self.grid, dims.CellDim, dims.KDim)
        self._interpolate_to_full_levels(
    ...
    def _make_axes(
        self, num_axes: int = -1, fig_num: int = 1
    ) -> tuple[mpl.figure.Figure, list[mpl.axes.Axes], list[mpl.axes.Axes]]:
        fig = plt.figure(fig_num)
        plt.clf()
        axs = fig.subplots(
            nrows=min(self.NUM_AXES_PER_COLUMN, num_axes),
            ncols=max(1, int(np.ceil(num_axes / self.NUM_AXES_PER_COLUMN))),
            sharex=True,
            sharey=True,
        )
        if num_axes > 1:
            ...
            axs = [axs]
        caxs = [
            make_axes_locatable(ax).append_axes("right", size="3%", pad=0.02)
            for ax in axs
        ]
        return fig, axs, caxs

    def plot_levels(
        self, data, num_levels: int = -1, label: str = "", fig_num: int = 1, qscale=40
    ) -> mpl.axes.Axes:
        """
        ...
        if cmin < 0 and cmax > 0:  # and np.abs(cmax + cmin) < cmax/3:
            cmap = "seismic"
            norm = lambda cmin, cmax: colors.TwoSlopeNorm(
                vmin=min(-1e-9, cmin), vcenter=0, vmax=max(1e-9, cmax)
            )
        else:
            if cmax > -cmin:
                cmap = "YlOrRd"
                ...
                cmap = "YlOrRd_r"
                # cmap = "gist_rainbow_r"
            norm = lambda cmin, cmax: colors.Normalize(
                vmin=min(-1e-9, cmin), vmax=max(1e-9, cmax)
            )

        match data.shape[0]:
            case self.grid.num_cells:
                plot_lev = lambda data, i: axs[i].tripcolor(
                    self.tri,
                    data[:, -1 - i],
                    edgecolor="none",
                    shading="flat",
                    cmap=cmap,
                    norm=norm(data[:, -1 - i].min(), data[:, -1 - i].max()),
                )
            case self.grid.num_edges:
                plot_lev = lambda data, i: axs[i].scatter(
                    self.tri.edge_x,
                    self.tri.edge_y,
                    c=data[:, -1 - i],
                    s=6**2,
        ...
        for i in range(num_levels):
            ...
            if "vvec_cell" in file_name:
                ...
                u = (
                    vn[:, -1 - i] * self.primal_normal[0]
                    + vt[:, -1 - i] * self.primal_tangent[0]
                )
                v = (
                    vn[:, -1 - i] * self.primal_normal[1]
                    + vt[:, -1 - i] * self.primal_tangent[1]
                )
    ...
    def plot_sections(
        ...
        data,
        data2=None,
        sections_x: list[float] = [],
        sections_y: list[float] = [],
        plot_every=1,
        qscale=40,
        label: str = "",
        fig_num: int = 1,
        ...
        if "vvec_cell" in file_name:
            plot_sec = lambda x, y, data, i: axs[i].scatter(
                x, y, c=data, s=6**2, cmap=cmap, norm=norm(data.min(), data.max())
            )
        else:
            plot_sec = lambda x, y, data, i: axs[i].pcolormesh(
                x,
                y,
                data[:-1, :-1],
                cmap=cmap,
                norm=norm(data.min(), data.max()),
                shading="flat",
            )
        ...
        for i in range(num_sections):
            if i < len(sections_x):
                if "vvec_cell" in file_name:
                    data = (v**2 + w**2) ** 0.5
                idxs = self._get_section_indexes(
                    coords_x,
                    coords_y,
                    s_x=sections_x[i],
                    dist=self.tri.height_length * 2 / 3,
                )
                ...
                idxs = self._get_section_indexes(
                    coords_x,
                    coords_y,
                    s_y=sections_y[i - len(sections_x)],
                    dist=self.tri.height_length * 2 / 3,
                )
                ...
                im = plot_sec(
                    x_coords[::pever, ::pever],
                    y_coords[::pever, ::pever],
                    data[idxs, :][::pever, ::pever],
                    i,
                )
                if "vvec_cell" in file_name:
                    quiver_sec(
            ...
            cbar.set_ticks(np.linspace(cbar.vmin, cbar.vmax, 5))
            cbar.set_ticklabels(
                [f"{c:.2f}" for c in np.linspace(cbar.vmin, cbar.vmax, 5)]
            )
            # axs[i].set_aspect('equal')
